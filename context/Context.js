...
export const ContextProvider = ({ children }) => {
  ...
  // Connect MetaMask (Подключение MetaMask )
  const connectWallet = async () => {
    //Check if browser have installed metamask (Проверка установлен ли в браузере MetaMask)
    if (!window.ethereum) return alert('Please install MetaMask !');

    //Request connection to metamask's user ethereum accounts (Запрос на подключение к учетным записям рользователей MetaMask ETH )
  ...
  //Create NFT (signer side)(Создание NFT )
